...
namespace example {
    ...
    inline void Tracker::setFirstFrame(const Mat frame, vector<Point2f> bb, string title, Stats& stats)
    {
    ...
    inline Mat Tracker::process(const Mat frame, Stats& stats)
    {
        TickMeter tm;
        vector<KeyPoint> kp;
        Mat desc;
    ...
    inline int tracking_on_photos(string photo, string photo_1) {

        string erasing = "file";
        size_t pos = photo.find(erasing);
        size_t pos_1 = photo_1.find(erasing);
        if ((pos != std::string::npos) && (pos_1 != std::string::npos)) {
            // If found then erase it from string
            photo.erase(pos, erasing.length());
            photo_1.erase(pos_1, erasing.length());
        }

        erasing = ":///";
        pos = photo.find(erasing);
        pos_1 = photo_1.find(erasing);
        if ((pos != std::string::npos) && (pos_1 != std::string::npos)) {
            // If found then erase it from string
            photo.erase(pos, erasing.length());
            photo_1.erase(pos_1, erasing.length());
        ...
        string video_name = "Tracking result";
        ...
        for (;;) {
            ...
            imwrite(photo + "_tracking.jpg", akaze_res);
